        ...
        <div style={{ display: 'flex', gap: 48, flex: 1 }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
              ...
              <span style={{ color: '#fb7185', fontSize: 18, fontWeight: 700 }}>오늘의 이슈</span>
            </div>
            {issueKws.map((kw) => (
              <div key={kw.word} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
                <span style={badge(kw.rank)}>{kw.rank}위</span>
                <span style={{
                  color: kw.rank === 1 ? 'white' : '#d4d4d8',
                  fontSize: kw.rank === 1 ? 26 : 20,
                  fontWeight: 700,
                }}>
                  {kw.word}
                </span>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
              <span style={{ fontSize: 20 }}>💰</span>
              <span style={{ color: '#60a5fa', fontSize: 18, fontWeight: 700 }}>경제</span>
            </div>
            {econKws.map((kw) => (
              <div key={kw.word} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
                <span style={badge(kw.rank)}>{kw.rank}위</span>
