...
stopwords_common = {
    ...
    # 연휴: 추석·설날·황금연휴처럼 항상 특정 명절명을 수식하는 분산 노이즈 — 명절명 자체가 더 구체적인 키워드
    '연휴',
    # 일요일: 매주 일요일 날씨·나들이 예보 기사에서 반복 추출되는 요일 명사 — 오늘과 같은 이유로 단독 키워드 가치 없음
    '일요일',
    ...
    # 기자: "이탈리아 기자"·"기자회견" 등 모든 카테고리에서 역할명사 또는 복합어 파편으로 추출되는 직함 노이즈 — 어떤 기자인지 특정 못함
    '기자',
    ...
    '통과', '발견', '귀국', '공개', '시작', '지속',
    # 발생: "연기 발생"·"사고 발생"·"화재 발생" 등 모든 카테고리에서 사건 유형 뒤에 붙는 행위명사 — 무슨 사건인지가 중요하지 '발생' 자체는 키워드 가치 없음(진행·실시와 같은 이유)
    '발생',
    # 공격: "정치 공격"·"사이버 공격"·"군사 공격" 등 누가 누구를 공격했는지가 중요하지 '공격' 자체는 키워드 가치 없음(발표·논의와 같은 이유)
    '공격',
    ...
    '방문',
    # 환영: "트럼프, 시진핑 공항서 환영"·"환영 행사" 등 외교·행사 기사마다 반복되는 행위명사 — 누가 누구를 환영했는지가 중요하지 '환영' 자체는 키워드 가치 없음(방문과 같은 이유)
    '환영',
    # 대화: "사회적 대화"·"남북 대화" 등 서로 무관한 외교·노동 기사에 흩어지는 분산 노이즈 — 어떤 주체가 어떤 대화에 참여했는지가 중요하지 '대화' 자체는 키워드 가치 없음(방문·논의와 같은 이유)
    '대화',
    ...
    '재개',
    # 전환: "이슈 전환"·"국면 전환"·"하락 전환"·"흑자전환" 등 정치·경제 등 모든 카테고리에서 방향 전환 행위명사로만 쓰이는 분산 노이즈 — 재개·추진과 같은 이유
    '전환',
    ...
    '확대', '이전', '강화',
    # 속도: "AI 개발 속도"·"건립 속도"·"시간제 속도" 등 서로 무관한 기사에 흩어지는 분산 노이즈 — 어떤 분야의 속도인지 특정 못함
    '속도',
    ...
    '변수',
    # 일반 명사 노이즈
    # 국민: "국민 존중"·"국민연금"·"국민 다섯 중 한 명" 등 서로 무관한 기사에 흩어지는 분산 노이즈 — 어떤 국민 이슈인지 특정 못함
    '국민',
    '뉴스', '한국', '지역', '사회', '국제', '관련', '사진', '인기', '본인',
    # 해외: 해외직구·해외연수·해외여행 등 서로 무관한 기사에 흩어지는 분산 노이즈 — 단독 키워드 가치 없음
    '해외',
    # 이야기: "강남불패는 과거 이야기가 될 것" 같은 반복 인용구에서 조각으로 추출되는 분산 노이즈 — 단독 키워드 가치 없음
    '이야기',
...
stopwords_economy = stopwords_common | {
    ...
    # 도로: 역세권 도로 확장·차량 화재·도로 개통 등 서로 무관한 지역 인프라·사고 기사에 흩어지는 분산 노이즈 — 경제 이슈가 아님
    '도로',
    # 통제: 내부통제·사법통제·AI 통제 등 서로 무관한 지배구조·사법·기술 기사에 흩어지는 분산 노이즈 — 무엇을 통제하는지가 중요하지 '통제' 자체는 경제 키워드 가치 없음
    '통제',
    '센터', '브리핑', '구윤철', '글로벌', '실용',
    '인사이트', '리포트', '뉴스룸', '핵심', '유통',
    '전략', '도입', '트렌드', '접목', '전망', '공략',
    ...
    '수도',
    # 오픈: "오픈AI"·"오픈런"처럼 더 긴 복합어/고유명사의 앞 파편으로 추출되는 분산 노이즈 — 단독으로 어떤 이슈인지 특정 못함(스포츠 stopwords와 같은 이유)
    '오픈',
}
...
stopwords_sports = stopwords_common | {
    ...
    '제압',
    # 안타·침묵·홈런·멀티: 타격/다득점 결과 기사에서 선수명 대신 추출되는 분산 노이즈 — 어떤 선수인지 특정 못함(승리·완승과 같은 이유)
    # 멀티: "이영준 멀티골"처럼 항상 뒤따르는 결과명사(골·안타)와 합성되어 쓰이는 성취 수식어 — 안타·홈런과 같은 이유로 단독 키워드 가치 없음
    '안타', '침묵', '홈런', '멀티',
    ...
    '기록', '도전', '대회', '메달', '은메달', '금메달', '동메달', '순위', '일정', '명단', '훈련', '소속',
    ...
    '미국', '중국', '일본', '북한', '영국', '러시아',
    # 대만: 스포츠 기사에서 뽑혀도 그 단어로 재검색하면 지정학·정치 뉴스가 딸려와 노이즈 — 미국·중국·일본과 같은 이유
    '대만',
    '아시아',  # 투자·에너지 기사에도 분산 등장 — 스포츠 구체 이슈를 특정하지 못함
    # 성별 수식어: 여자배구·남자축구처럼 항상 더 긴 복합어의 수식어로만 쓰임 — 단독 키워드 가치 없음
    # 남녀: 남녀 복식·남녀 경기처럼 성별 구분 수식어로만 쓰임 — 여자/남자와 같은 이유
    '여자', '남자', '남녀',
    ...
    '괴물',
    # 금빛: "금빛 찌르기"·"금빛 레이스" 등 금메달 성취를 수식하는 시적 표현 — 작렬과 같은 이유로 실제 주체(선수명·종목)가 키워드이지 '금빛' 자체는 키워드 가치 없음
    '금빛',
    # 운명: "운명의 한일전"처럼 극적 맥락에서 수식어로만 쓰이는 명사 — 작렬·금빛과 같은 이유로 어떤 경기인지가 중요하지 '운명' 자체는 키워드 가치 없음
    '운명',
    # 출발: "새 출발"·"상승 출발" 등 어떤 종목·팀이든 붙는 시작 행위명사 — 진행·실시와 같은 이유로 단독 키워드 가치 없음
    '출발',
}
