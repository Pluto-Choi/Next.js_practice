...
stopwords_common = {
    # 시간
    '이후', '올해', '내년', '지난', '이번', '최근', '오늘', '내일', '다음', '출근길', '이제',
    # 시간: "4시간 대기"·"24시간 거래" 등 항상 수치 뒤 단위로만 쓰이는 분산 노이즈 — 단독 키워드 가치 없음
    '시간',
    # 직후: "감금한 직후"·"홈런 직후"·"사고 직후" 등 모든 카테고리에서 시간 순서 연결어로만 쓰이는 분산 노이즈 — 이후와 같은 이유로 단독 키워드 가치 없음
    '직후',
    # 언론사
    '한겨레', '조선', '중앙', '동아', '연합', '일보', '신문', '방송',
    '뉴시스', '네이트', '내외', '연합인포맥스', '아시아경제', '브릿지',
    '한국경제', '불교방송', '헤럴드', '파이낸셜포스트', '지디넷코리아',
    '경남일보', '연합뉴스', '시사저널', '아이티데일리', '조선비즈', '머니투데이',
    # 매체: "현지 매체", "스페인 매체" 등 출처를 가리키는 범용 상위어 — 신문·방송과 같은 이유로 단독 키워드 가치 없음
    '매체',
    # 직함
    '대통령', '총리', '장관', '차관', '의원', '대표', '회장', '사장',
    '부장', '위원', '본부', '수석', '시장', '감독', '청장',
    # 정상: "브릭스 정상"·"정상회의" 등 외교 기사마다 반복되는 직급 수식어 — 어느 지도자인지 특정 못함
    '정상',
    # 고위: "고위 당정"·"고위직 비리"·"고위급 외교 회담" 등 서로 무관한 기사에
    # 수식어로만 등장하는 직급 수식어 — 강력/공식/본격과 같은 이유
    '고위',
    # 행위 노이즈
    '개최', '점검', '회의', '공동', '진행', '추진', '실시', '투자',
    '참석', '참가', '발표', '논의', '강조', '예정', '확인', '지원', '지지', '운영', '응원',
    '통과', '발견', '귀국', '공개', '시작', '지속',
    # 관리: 교도소 탄약 관리·국공유지 관리·수면무호흡 관리 등 서로 무관한 기사에 흩어지는 행위명사 — 추진/운영과 같은 이유로 무엇을 관리하는지가 중요하지 '관리' 자체는 키워드 가치 없음
    '관리',
    # 취소: 공연 취소·계약 취소·공소 취소 등 서로 무관한 기사에 흩어지는 행위명사 — 무엇이 취소됐는지가 중요하지 '취소' 자체는 키워드 가치 없음
    '취소',
    # 입장: "다양한 입장이 충돌"·"공식 입장" 등 항상 다른 명사를 수식하는 범용 상태명사 — 단독 키워드 가치 없음
    '입장',
    # 추정: "자살 추정"·"목맴사 추정" 등 항상 앞 명사를 수식하는 범용 판단명사 — 진행·추진과 같은 이유
    '추정',
    # 예상: "금리인상 예상 웃돌아"·"인기 예상 못 해" 등 경제·연예·스포츠 등 서로 무관한 기사에서 전망·예측 수식어로만 쓰이는 분산 노이즈 — 추정과 같은 이유
    '예상',
    # 의지: "인상 의지", "안정 의지" 등 항상 다른 동사·명사를 수식하는 행위 지지명사 — 진행·추진과 같은 이유로 단독 키워드 가치 없음
    '의지',
    # 방문: "왕이 방한", "의장 방문", "대통령 방문" 등 서로 무관한 외교·행사 기사에
    # 흩어지는 행위명사 — 누가 어디를 방문했는지가 중요하지 '방문' 자체는 키워드 가치 없음
    '방문',
    # 공식: "공식 방문", "공식 발표", "공식 입장" 등 항상 더 긴 복합어의 수식어로만 쓰이는 분산 노이즈
    # — 강력·추가·일부와 같은 이유로 단독 키워드 가치 없음
    '공식',
    # 본격: "단속 본격"·"협력 본격화"·"도입 본격" 등 모든 분야의 신규 사업·조치 기사마다 반복되는 행위 수식어 — 추진/실시와 같은 이유
    '본격',
    # 등장: "황정민 등장"·"새 CEO 등장" 등 누가/무엇이 등장했는지보다 '등장'이라는 행위만 추출됨 — 출연(연예)과 같은 이유
    '등장',
    # 개발: 치료제 개발·예후 모델 개발 등 서로 무관한 R&D 기사에 흩어지는 행위명사 — 추진/실시와 같은 이유
    '개발',
    # 준비: "배치 준비", "수용 준비", "행동할 준비" 등 항상 다른 명사를 수식하는 행위명사 — 주체·사건 특정 불가
    '준비',
    # 대비: 태풍·전쟁·수능 등 서로 무관한 기사에 '~에 대비' 형태로만 등장하는 수식 행위명사
    '대비',
    # 대신: "A 대신 B" 형태의 순수 관계어 — "안영준 대신 문정현"·"현금 대신 채권" 등 서로 무관한 기사에 흩어지는 분산 노이즈 — 대비와 같은 이유로 단독 키워드 가치 없음
    '대신',
    # 강력: "강력 응징", "강력 반발" 등 모든 카테고리 헤드라인에서 수식어로만 쓰이는 분산 노이즈
    '강력',
    # 처리: 법안·폐수·업무 등 서로 무관한 맥락에 흩어지는 행위명사 — 추진/실시와 같은 이유
    '처리',
    # 재개: 전쟁·협상·경기·운행 등 서로 무관한 맥락에 흩어지는 행위명사
    '재개',
    '도약', '민간', '공공', '분야', '상황', '문제', '방안',
    '계획', '목표', '효과', '결과', '사업', '행사', '기관', '현장',
    '대응', '혁신', '시행', '후보', '선거', '전시', '총력',
    '의혹', '논란', '책임', '완료', '확정', '조사', '보유', '연속', '투입', '검토',
    '확대', '이전', '강화',
    # 정책: 결혼·국방·기후 등 서로 무관한 정책 기사마다 반복되는 메타어 — 논란·의혹과 같은 이유
    '정책',
    # 변화: 표면 변화·인식 변화·정책 변화 등 모든 카테고리에서 수식어로만 등장하는 분산 노이즈 — 단독 키워드 가치 없음
    '변화',
    # 제한: 대통령 연임 제한·학생부 이용 제한·레버리지 투자 제한 등 서로 무관한 정책 맥락에 흩어지는 행위명사 — 추진/실시/처리와 같은 이유
    '제한',
    # 추가: "추가 파병", "4곳 추가", "추가 등재" 등 분량·범위 한정사로만 쓰이는 분산 수식어
    '추가',
    # 일부: "일부 존치"·"일부 허용"처럼 분량·범위 한정사로만 쓰임 — 여러 무관한 사안의 수식어로 분산
    '일부',
    # 전원: "민주당 의원 전원 초청"·"가족 전원 사망" 등 항상 다른 명사를 수식하는 수량 한정사 — 추가·일부와 같은 이유로 단독 키워드 가치 없음
    '전원',
    # 선언: 출마·은퇴·전쟁 선언 등 무관한 맥락에 분산(발표·공개·강조와 동일 이유)
    # 혐의: 사기·횡령·선거법 등 서로 무관한 법조 사건마다 단독 추출되는 분산 노이즈
    # 사퇴: 정치·스포츠·기업 등 모든 카테고리의 사임·사퇴 기사에 분산되는 행위명사
    # 구속: 살인·횡령·선거법 등 서로 무관한 법조 사건마다 단독 추출되는 분산 노이즈 — 혐의·사퇴와 같은 이유로 누가 구속됐는지가 중요하지 '구속' 자체는 키워드 가치 없음
    # 출시: 스마트폰·금융상품·게임 등 서로 무관한 신제품 기사에 흩어지는 분산 노이즈 — LLM 프롬프트에서도 제거 대상으로 명시된 런칭 행위명사
    '선언', '혐의', '사퇴', '구속', '출시',
    # 이견: 협상·정치·스포츠 등 서로 무관한 대립/합의 기사마다 반복되는 상태명사 — 논란·의혹과 같은 이유
    '이견',
    # 연쇄: '연쇄 회동·살인·파업·부도' 등 항상 다른 명사를 수식하는 수식 명사 — 단독 키워드 가치 없음
    '연쇄',
    # 가능·계속: 수식·접속 기능어 — 단독 키워드 가치 없음(32강 가능성·비트코인 계속 등 무관 기사에 분산)
    '가능', '계속',
    # 박사: 직함·학위 명사 — 단독으로 사건 주체를 특정하지 못함
    '박사',
    # 전문가: 직함·역할 명사 — "미장 전문가", "부동산 전문가" 등 서로 무관한 기사에 흩어지는 분산 노이즈
    '전문가',
    # 수식·클릭베이트성 노이즈(광고문구·헤드라인 감탄어에서 단독 추출되는 형용사성 명사)
    '완벽', '화제', '눈길', '눈앞', '포착', '충격', '깜짝', '감동', '대박', '눈물', '극찬',
    # 근황: "유퉁 근황"·"이효리 근황" 등 연예 기사가 이슈·스포츠 피드로 새어들 때
    # 단독 추출되는 메타어 — 실제 키워드는 그 인물명이지 '근황'이 아님
    '근황',
    # 수식 형용사 파편(가짜뉴스·가짜 국가대표 등 항상 더 긴 복합어의 수식어로만 쓰임)
    '가짜',
    # 역대·사상·최대·최고·최다: "역대 최고", "사상 최대", "최대 실적", "최다 점수 차" 등 최상급·극값 수식어로만 쓰이는 일반 명사 — 단독 키워드 가치 없음
    '역대', '사상', '최대', '최고', '최다',
    # 하락·급등·급증·상승: 주가·유가·지지율·환율 등 서로 무관한 여러 지표의 등락을 뭉뚱그리는 분산 노이즈
    # (무엇이 올랐는지·내렸는지가 중요하지 방향 자체는 키워드 가치 없음)
    '하락', '급등', '급증', '상승',
    ...
    '프로그램', '대책', '사람', '위험', '정부', '법원',
    # 최소(最少): "최소 5명 사망"·"최소 예산"·"최소 13일까지"처럼 수량 기준 수식어 — 이상·추가와 같은 이유로 단독 키워드 가치 없음
    '최소',
    # 이상(以上): "300㎜ 이상"·"30억 이상"처럼 수량 기준 뒤에 붙는 순수 수식어 — 단독 키워드 가치 없음
    '이상',
    # 정도: "불편한 정도"·"될 정도"·"어느 정도" 등 모든 카테고리에서 기준·수식어로만 쓰이는 분산 노이즈 — 이상·최소와 같은 이유로 단독 키워드 가치 없음
    '정도',
    # 기준: "기초연금 지급 기준"·"기준금리"·"종부세 기준" 등 서로 무관한 정책 도메인에 흩어지는 분산 수식명사
    '기준',
    # 나라: '한국'·'세계'처럼 단독으로는 어느 나라인지 특정하지 못하는 분산 노이즈;
    # 또한 '배나라' 같은 인명의 형태소 조각으로도 추출됨
    '나라',
    # 운세: 지윤철학원 등 매일 발행되는 띠별 운세 콘텐츠 — 뉴스 이슈가 아닌 반복성 노이즈
    '운세',
    # 우려: 폭염 우려·보완수사권 폐지 우려·사고 우려 등 서로 무관한 기사에 흩어지는 분산 노이즈
    '우려',
    # 긴장·여론: "중동 긴장", "K-바이오 긴장" 등 분산 상태어 / "여론조사 여론" 등 여러 정치·경제·사회 기사에 흩어지는 메타어 — 어떤 사건인지 특정 못함
    '긴장', '여론',
    # 부담: "봐주기 부담"·"살라미식 폭로가 부담" 등 정치·사회 기사에서 수식어로만 쓰이는 분산 상태명사 — 우려와 같은 이유로 단독 키워드 가치 없음
    '부담',
    # 주목: "주목받는 이유"·"日 매체도 주목" 등 모든 카테고리 헤드라인에서 관심 표현 수식어로만 쓰이는 분산 노이즈 — 화제·눈길과 같은 이유
    '주목',
    # 감정·심리 일반명사: 사과·인터뷰 헤드라인에서 단독 추출되지만 어떤 사건인지 특정 못함
    '마음', '사랑',
    # 생각: "MLS 이적 생각 없었다"·"다른 생각 없다" 등 인용·심리 맥락에서만 추출되는 분산 노이즈
    # — 어떤 사건인지 특정 못함(마음·사랑과 같은 이유)
    '생각',
    # 질문: "감독의 질문"·"꽃문신 논란 질문" 등 의회·인터뷰·영화 비평 등 서로 무관한 기사에
    # 흩어지는 분산 노이즈 — 어떤 사건인지 특정 못함(생각·마음과 같은 이유)
    '질문',
    # 인생: "배구 인생 23점"·"내 인생 최고 경기" 등 선수·연예인 인터뷰·프로필 기사마다 반복되는 메타어
    # — 마음·근황과 같은 이유로 어떤 인물인지가 중요하지 '인생' 자체는 키워드 가치 없음
    '인생',
    # 피해: "파병 피해는 국민에게"·"사고 피해"·"세금 피해" 등 정치·사회·경제 등 서로 무관한 기사에
    # 흩어지는 분산 상태명사 — 어떤 피해인지 특정 못함(우려·부담과 같은 이유)
    '피해',
    # 세력: "내란세력"·"친윤세력"·"야권 세력" 등 서로 무관한 정치 기사마다 반복되는 집단 수식어
    # — 어느 세력인지 특정 못함(여론·긴장과 같은 이유로 단독 키워드 가치 없음)
    '세력',
    ...
    '필요', '개선', '마비', '신고',
    # 보호: 피해자 보호·수원보호구역·교권보호 등 서로 무관한 기사에 흩어지는 분산 행위명사 — 지원/강화와 같은 이유
    '보호',
    # 신청: 노령연금·근로장려금·국민연금 추납 등 서로 무관한 복지·행정 신청 기사에 흩어지는 행위명사 — 신고/발표와 같은 이유
    '신청',
    # 발언·주장: 정치·스포츠 등 여러 카테고리에서 "누군가 발언/주장" 형태로 분산되는 행위명사 — 발표와 같은 이유
    '발언', '주장',
    # 작렬: "뒤끝 작렬"(정치)·"2루타 작렬"(야구)·"연속골 작렬"(축구) 등 모든 카테고리에서
    # 극적 효과 수식어로만 쓰이는 분산 노이즈 — 단독으로 뉴스 주제를 특정하지 못함
    '작렬',
    # 기사 형식 태그
    '속보',
    ...
    '아들',
    # 부부: "윤석열 부부"·"결혼지옥 김밥부부" 등 항상 특정 인물쌍의 수식어로만 쓰이는 분산 노이즈
    # — 아들과 같은 이유로 단독 키워드 가치 없음
    '부부',
    # 아내: 범죄·사회 등 여러 카테고리에서 가족 관계 언급 시 단독 추출되는 분산 노이즈 — 아들·부부와 같은 이유로 단독 키워드 가치 없음
    '아내',
    # 이슈: "심각한 이슈", "주요 이슈" 처럼 기사 문체에서 단독 추출되는 메타어 노이즈
    '이슈',
    # 생애: "생애 첫…", "생애 최고…" 형태로 어떤 카테고리에서든 수식어로만 쓰임 — 단독 키워드 가치 없음
    '생애',
    # 소리: "쓴소리", "패배수 소리" 등 관용어·표현의 파편 — 단독으로 뉴스 주제를 특정하지 못함
    '소리',
    # 최초·처음: "한국 투수 최초", "역사상 최초", "처음으로 X" 등 역대·사상·최대·최고와 같은 최상급 수식어 — 단독 키워드 가치 없음
    '최초', '처음',
    # 프로: '서피스 프로 12·프로야구·프로팀' 등 합성어·복합어에서 쪼개진 파편 — 단독 키워드 가치 없음
    '프로',
    # 영화: 이슈·경제 피드에서 특정 작품이 아닌 비유·수식어로 등장하는 분산 노이즈 — 작품명 자체가 진짜 키워드
    '영화',
    # 친구: 연애·사고·사연 등 서로 무관한 기사에 관계명사로만 쓰이는 분산 노이즈 — 단독으로 이슈 특정 불가
    '친구',
}
...
stopwords_economy = stopwords_common | {
    ...
    '가동', '장기',
    '자금',
    # 수요: 전력·물가·이전 수요 등 서로 무관한 기사에 흩어지는 분산 노이즈 — 어떤 수요인지 단독으로 특정 못함
    '수요',
    # 경제 방향성 동사명사(무엇이 올랐는지/내렸는지가 중요하지 오름·내림·조정 자체는 키워드가 아님)
    # 상승은 stopwords_common으로 이동 — 경제뿐 아니라 모든 카테고리에서 방향어로 쓰임
    '폭락', '급락', '성공',
    # 상향: "비중 확대로 상향"·"목표가 상향"·"예탁금 상향" 등 서로 무관한 증권 분석 기사에 흩어지는 행위명사 — 상승/급락과 같은 이유
    '상향',
    # 주가: "SK하이닉스 주가·샤오미 주가·솔라리스 주가" 처럼 서로 무관한 종목 기사에
    # 흩어지는 분산 노이즈 — 어느 주식인지 특정하지 못함(상승/폭락과 같은 이유)
    '주가',
    # 주식: "애국 주식 폭락·오너 주식 매입·바이오텍 주식 선정" 등 서로 무관한 종목·투자 기사에
    # 흩어지는 분산 노이즈 — 주가와 같은 이유로 단독 키워드 가치 없음
    '주식',
    ...
    '미국',
    # 돌파: "유가 100달러 돌파"·"SK하이닉스 180만 원 돌파"·"700만명 돌파" 등 서로 무관한 지표·기준 기사에서 임계값 초과 행위어로만 쓰이는 분산 노이즈 — 상승/폭락과 같은 이유로 무엇이 돌파됐는지가 중요하지 '돌파' 자체는 키워드 가치 없음
    '돌파',
    # 업체: "차량용 반도체 업체"·"특정 업체 봐주기" 등 서로 무관한 기업·계약 기사에 흩어지는 분산 노이즈 — 기업과 같은 이유로 어느 업체인지 단독으로 특정하지 못함
    '업체',
    ...
    '허가',
    # 인상: 금리·식품값·전기요금 등 서로 무관한 인상 기사에 흩어지는 분산 노이즈 — 상승/급락과 같은 이유
    '인상',
    # 전환: "하락 전환"·"ADR 상호 전환"·"흑자전환" 등 서로 무관한 경제 기사에 흩어지는 방향성 행위명사 — 상승/폭락과 같은 이유
    '전환',
    # 약세·강세: "증시 약세", "원전주 강세" 등 방향성 시세어 — 상승/폭락/급락과 같은 이유
    '약세', '강세',
    # 인수: "KKR 인수", "SK하이닉스 인수" 등 서로 무관한 M&A 기사에 흩어지는 행위명사 — 협상과 같은 이유
    '인수',
    # 가격: 유가·AI 서비스·식품 가격 등 서로 무관한 여러 가격 기사에 흩어지는 분산 노이즈 — 어느 가격인지 특정 못함
    '가격',
    # 현금: 투자 규제·재산분할·세금 등 서로 무관한 기사에 흩어지는 분산 노이즈 — 어떤 현금 이슈인지 특정 못함
    '현금',
    # 영업: "영업정지"·"불법영업"·"영업이익" 등 서로 무관한 규제·단속·실적 기사에 흩어지는 행위명사 — 어떤 사건인지 특정 못함
    '영업',
    ...
    '의견',
    # 기대: "기대 이하"·"기대감에 랠리"·"기대보다 실망" 등 모든 경제 기사에서 감정·평가 수식어로만 쓰이는 분산 노이즈
    # — 어떤 자산/사건인지 특정 못함(우려와 같은 이유)
    '기대',
    # 업종: "집중할 업종·종목"·"이업종 교류"·"주력산업 업종 재편" 등 서로 무관한 산업·투자 기사에 흩어지는 분산 노이즈
    # — 어떤 업종인지 단독으로 특정하지 못함(종목·주가와 같은 이유)
    '업종',
    # 정보: "암살미수 정보 숨겨"·"개인정보 유출 우려"·"의료정보 활용" 등 서로 무관한 정치·국제·보건 기사가
    # 경제 피드로 유입될 때 단독 추출되는 범용 명사 — 어떤 정보인지 특정 못함
    '정보',
    # 수도: "행정수도특별법"(입법)·"해양수도특별법"(입법)처럼 경제와 무관한 정치 기사에서 나오는 데다,
    # "살 수도 있습니다"처럼 보조동사 구문 '수도 있다'에서 Kiwi 오추출되는 형태소 잔재
    '수도',
}
...
stopwords_entertainment = stopwords_common | {
    ...
    '자택', '댓글', '성공', '사태', '스타이슈',
    # 픽: mhnse.com "[MHN:픽]" 섹션 태그에서 추출되는 컬럼명 파편 — 뉴스 이슈가 아닌 언론사 내부 라벨
    '픽',
    # 이웃: 노래 제목·법원 판결·생활정보 기사에 흩어지는 분산 노이즈 — 연예 키워드 아님
    '이웃',
    # 이상형: 연예 인터뷰 단골 질문(이상형이 누구냐)에서 반복 추출되는 일반명사 — 특정 인물·사건이 아님
    '이상형',
    # 아파트: 부동산·대출규제 기사가 연예 피드에 유입되는 경제 노이즈 — 연예 콘텐츠 아님
    '아파트',
    # 스윙: "스윙스"(래퍼)·"코브스윙GC"(골프장)·"헛스윙"(야구) 등 서로 무관한 기사에서 내부 파편으로 추출되는 형태소 잔재 — 단독 키워드 가치 없음
    '스윙',
}
...
stopwords_sports = stopwords_common | {
    ...
    '스포츠', '경기', '상대', '선수', '코치', '구단', '대표팀', '국가대표',
    '우승', '준우승', '승리', '패배', '무승부', '승부', '연승', '연패', '득점', '실점',
    # 도움: "1호 도움"·"첫 도움" 등 서로 다른 선수 기사에 흩어지는 어시스트 행위명사 — 득점/실점과 같은 이유
    '도움',
    '출전', '출격', '선발', '교체', '복귀', '부상', '은퇴', '이적', '영입', '계약', '발탁', '합류', '입단',
    # 소집: "A매치 소집 명단 발표"처럼 대표팀/단체 구성 때마다 반복되는 행위명사 — 발탁·합류·입단과 같은 이유로 단독 키워드 가치 없음
    '소집',
    '탈락', '진출', '실패',
    # 완승: "3-1 완승", "4-0 완승" 등 서로 무관한 경기 결과 기사마다 반복되는 행위명사 — 승리와 같은 이유
    '완승',
    # 역전승: "8-6 역전승"·"연장 역전승" 등 서로 무관한 경기 결과 기사마다 반복되는 행위명사 — 승리·완승과 같은 이유
    '역전승',
    # 완파: "3-0 완파", "10년 만의 완파" 등 서로 무관한 대승 기사마다 반복되는 결과명사 — 완승과 같은 이유
    '완파',
    # 제압: "KIA 6-2 제압"·"3-0 제압" 등 서로 무관한 경기 결과 기사마다 반복되는 행위명사 — 완승·완파와 같은 이유
    '제압',
    # 안타·침묵·홈런: 타격 결과 기사에서 선수명 대신 추출되는 분산 노이즈 — 어떤 선수인지 특정 못함(승리·완승과 같은 이유)
    '안타', '침묵', '홈런',
    # 타자: "1할 타자 김하성"·"타자 농락" 등 선수명 대신 추출되는 역할 명사 — 선수·코치·구단과 같은 이유로 단독 키워드 가치 없음
    '타자',
    # 타점: "2타점 적시타"·"56타점" 등 서로 다른 선수 기사에 흩어지는 타격 결과 명사 — 안타·홈런과 같은 이유
    '타점',
    # 도루: "도루 신기록"·"도루 성공률" 등 서로 다른 선수 기사에 흩어지는 타격 행위명사 — 안타·홈런·타점과 같은 이유로 어떤 선수인지가 중요하지 '도루' 자체는 키워드 가치 없음
    '도루',
    # 성공: "도루 성공"·"성공한다면" 등 스포츠 성취 기사마다 반복되는 결과·행위 수식어 — stopwords_economy·entertainment와 같은 이유(우승·완승과 동일)
    '성공',
    '후반', '전반', '추가시간',  # 경기 시간대 명사 — 어떤 경기인지 특정 못함
    '리그', '결승', '결승전', '준결승', '예선', '본선', '개막', '폐막', '경기장',
    '기록', '도전', '대회', '메달', '순위', '일정', '명단', '훈련', '소속',
    # 타이틀: "타이틀 방어"·"타이틀 매치" 등 항상 앞뒤 명사를 수식하는 대회 유형 명사 — 결승·대회와 같은 이유
    '타이틀',
    # 오픈: "KG 레이디스 오픈"·"US 오픈" 등 특정 대회명의 뒤에 붙는 대회 유형 명사 — 단독으로 어느 대회인지 특정 못함
    '오픈',
    # 시즌·인터뷰: 서로 다른 종목·선수 기사에 흩어지는 분산 노이즈(연예 카테고리와 동일 이유로 이미 필터)
    '시즌', '인터뷰',
    # 종목명은 그날의 구체 이슈(선수·대회)가 아니라 섹션 분류라 거른다.
    '야구', '축구', '농구', '배구', '골프', '테니스', '배드민턴', '탁구', '육상',
    '프로야구', '프로축구', '프로농구', '메이저리그', '프리미어리그',
    # '프로'는 stopwords_common으로 이동 — 모든 카테고리에서 복합어 파편으로 나타남
    # 기대·희망: 선수·경기 예고 기사마다 반복되는 일반명사 — 어떤 선수/경기인지 특정 못함
    '기대', '희망',
    # 악재: "핵심 선수 결장 악재"·"부상 악재" 등 팀 상황 기사마다 반복되는 상태명사 — 어느 팀의 어떤 악재인지보다 선수/팀명이 실제 키워드
    '악재',
    # 국가·아시안·라운드: 복합어 내부 파편 — '국가대표'·'아시안컵'·'1라운드 지명' 등에서
    # 분리 추출되는 형태소 잔재 — 단독 키워드 가치 없음(국가대표·대표팀은 이미 필터됨)
    '국가', '아시안', '라운드',
    # 통산: "한미 통산 2500탈삼진", "통산 300승" 등 기록 집계 수식어 — 어떤 선수/기록인지 특정 못함
    '통산',
    ...
    # 성별 수식어: 여자배구·남자축구처럼 항상 더 긴 복합어의 수식어로만 쓰임 — 단독 키워드 가치 없음
    '여자', '남자',
    # 최연소 파편: '최연소 X승' 기사에서 Kiwi가 '최'를 분리하고 '연소'만 단독 추출하는 형태소 잔재
    '연소',
    # 선수 인터뷰 심리어: "의식했다"·"무의식적으로" 등 멘탈 언급 인용구에서 추출되는 일반명사
    '의식',
    # 게임: 아시안게임·올림픽게임 등 복합어에서 쪼개지는 형태소 파편 — 경기와 같은 이유로 단독 키워드 가치 없음
    '게임',
    # 결혼: 국제결혼 피해 사건·사회 기사가 스포츠 피드로 유입되는 노이즈 — 스포츠 이슈가 아님(연예 stopwords와 동일 이유)
    '결혼',
    # 역사: "새 역사를 썼다"·"역사 쓸 것" 등 스포츠 성취 기사마다 반복되는 마일스톤 수식어 — 우승·완승과 같은 이유로 어떤 선수/기록인지가 중요하지 '역사' 자체는 키워드 가치 없음
    '역사',
    # 달성: "5억 달성"·"3연패 달성" 등 서로 무관한 스포츠 성취 기사마다 반복되는 행위명사 — 우승·완승과 같은 이유로 무엇을 달성했는지가 중요하지 '달성' 자체는 키워드 가치 없음
    '달성',
    # 선두: "다승 단독 선두" 등 순위 위치어 — 순위와 같은 이유로 누가 선두인지가 중요하지 '선두' 자체는 키워드 가치 없음
    '선두',
    # 하위: "하위팀 반란"·"하위권 혼전" 등 순위 위치어 — 선두와 같은 이유로 단독 키워드 가치 없음
    '하위',
    # 순항: "3연패 순항"·"8강 순항" 등 '잘 되고 있다'는 상태 수식어 — 완승·역전승과 같은 이유로 누가 순항 중인지가 중요하지 '순항' 자체는 키워드 가치 없음
    '순항',
    # 2루타: "김하성 2루타"처럼 특정 타격 결과 기사에서 선수명 대신 추출되는 타격 행위명사 — 안타·홈런과 같은 이유로 어떤 선수인지가 중요하지 타구 종류 자체는 키워드 가치 없음
    '2루타',
    # 괴물: "괴물 좌완"·"괴물 타자"처럼 선수·팀을 극찬하는 수식성 감탄어 — 깜짝·충격과 같은 이유로 실제 주체(선수명)가 키워드이지 '괴물' 자체는 키워드 가치 없음
    '괴물',
}
